...
def evaluate(
        ...
        retrieved_thr, chatgpt_qa_thr, scorer,
        cluster=True, kernel=40, verbose=True, sample=True):
...
def evaluate_vanila(
        test_retrieved_scores,
        test_queries, test_answers, test_chatgpt_answers, 
        test_chatgpt_semantic_ids, test_chatgpt_probs, scorer,
        cluster=True, kernel=40, verbose=True, sample=True):
...
def objective(w1, w2):
    ...
    results = evaluate(cal_second_retrieved_scores, cal_second_queries, 
                       ...
                       retrieved_thr, chatgpt_qa_thr, scorer=scorer,
                       cluster=True)
...
if __name__ == '__main__':
    ...
    results = evaluate_vanila(
        ...
        chatgpt_semantic_ids, chatgpt_probs, scorer=scorer,
        cluster=True)
    ...
    with open("../collected_results/chatgpt_results_new.txt", "a") as f:
