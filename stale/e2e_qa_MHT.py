...
def evaluate(indices_list, answers_list,
    ...
    unique_answer_sets = []
    semantic_clusters = []
    retrieve_tolerate = w0 / (2/alpha - w1)
    requests = []
...
def main():
    ...
    seeds = list(range(6, args.num_seed+6))
    ...
    for seed in seeds:
        ...
        thr_qa = np.quantile(cal_true_scores, args.alpha/2)
        ...
        #                    args, dpr_records,
        #                    most_relevant_scores, cal_true_scores,
        #                    scorer, 1/2, 1/2, alpha=alpha, save=True)
        results = evaluate(test, test_answers,
                           test_retrieve_scores,
                           args, dpr_records,
                           most_relevant_scores, cal_true_scores,
                           scorer, 1.0, 1.0, alpha=alpha, save=True)
        ...
        def my_objective_function(w1, w2):
            weights = softmax(np.array([w1, w2])).tolist()
            results = evaluate(calibration, train_answers,
                               train_retrieve_scores,
                               args, dpr_records,
                               most_relevant_scores, cal_true_scores,
                               scorer, w0=weights[0], w1=weights[1],
                               alpha=alpha)
            metric = results[3]
            return metric
        ...
        # weights = softmax(np.array([0.62776441, 0.37223559]))
        results = evaluate(test, test_answers,
                                  test_retrieve_scores,
                                  args, dpr_records,
                                  most_relevant_scores, cal_true_scores,
                                  scorer, weights[0], weights[1],
                                  alpha=alpha, save=True, task='weighted')
